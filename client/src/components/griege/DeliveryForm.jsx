...
export default function DeliveryForm() {
  ...
  const isAnyValidRow =
    globalColor &&
    rows.some(
      (row) => finishingValues[row.row] && !isNaN(finishingValues[row.row])
    );
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 bg-gray-50 shadow rounded">
      ...
      {matchedDemand && (
        <div className="bg-white p-4 border rounded mb-4 space-y-2 text-sm">
          <p>
            <strong>গ্রেঃ গ্রহনের তারিখ:</strong>{" "}
            {new Date(matchedDemand["Date"]).toLocaleDateString()}
          </p>
        ...
        <>
          <div className="mb-4">
            <label className="block text-sm font-medium mb-1">
              Select Color for All Rows with Finishing
            </label>
            <select
              value={globalColor}
          ...
          {globalColor && (
            <div className="text-sm text-gray-700 mb-3">
              ✅ Selected Color for Submission:{" "}
              <strong className="text-blue-600">{globalColor}</strong>
            </div>
          )}
      ...
      {rows.length > 0 && (
        <button
          onClick={handleSubmit}
          disabled={!isAnyValidRow || submitting}
          className={`px-6 py-2 rounded text-white ${
            isAnyValidRow
              ? "bg-green-600 hover:bg-green-700"
              : "bg-gray-400 cursor-not-allowed"
