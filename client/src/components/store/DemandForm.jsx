...
const DyesDemandForm = () => {
  ...
  useEffect(() => {
    watchColors.forEach((item, index) => {
      if (item?.colorName && colorPrice[item.colorName]) {
        setValue(`colors.${index}.price`, colorPrice[item.colorName]);
        if (!stockColor.includes(item.colorName)) {
          setStockColor((prev) => [...prev, item.colorName]);
        }
      } else {
        setValue(`colors.${index}.price`, "");
      }
  ...
  const stockUpdate = async (qty) => {
    try {
      const response = await fetch(`https://hcml-ry8s.vercel.app/stock`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ color: stockColor, qty: parseInt(qty) }),
      });
  ...
  const onSubmit = (data) => {
    setLoading(true);
    const totalCost = data.colors.reduce(
      (sum, item) =>
        sum + parseFloat(item.gram || 0) * parseFloat(item.price || 0),
      0
    );
  ...
  const lotStatus = useMemo(() => {
    const status = { initialQty: 0, Dyeing: 0, Printing: 0, Finishing: 0 };
    if (!watchLot || verifyDyes.length === 0 || demandData.length === 0)
      return status;
    ...
    verifyDyes.slice(1).forEach((row) => {
      if (String(row[lotIndex]) === String(watchLot)) {
        const workType = row[workTypeIndex];
        const quantity = parseFloat(row[qtyIndex]) || 0;
        if (status[workType] !== undefined) status[workType] += quantity;
      }
    });
  ...
  const getWarningMessage = useCallback(() => {
    ...
    if (!watchWorkType) return "";

    const done = lotStatus[watchWorkType];
    const remaining = lotStatus.initialQty - done;

    if (qty > remaining) {
      messages.push(
        `এই লটে গ্রে ${watchWorkType} বাকি আছে ${remaining} গজ। এখন পর্যন্ত ${watchWorkType} হয়েছে ${done.toFixed(
          2
        )} গজ`
      );
    } else {
      messages.push(`এই লটে গ্রে ${watchWorkType} বাকি আছে ${remaining} গজ`);
    }
    return messages.join("\n");
  }, [lotStatus, watchQty, watchWorkType]);
