...
const DyesDemandForm = () => {
  ...
  useEffect(() => {
    fetch(`https://hcml-ry8s.vercel.app/demand/verifydyes`)
      ...
      .catch((error) => {
        console.error("Failed to fetch verify dyes:", error);
      });
  }, []);
  ...
  const stockUpdate = async () => {
    ...
    if (
      ...
      watchColors.every((c) => !c.colorName || !c.gram)
    ) {
      setMessage("❌ No valid color data to submit.");
      return false;
    }

    try {
      console.log("Submitting colors to stock API:", watchColors);
      const response = await fetch("https://hcml-ry8s.vercel.app/stock", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ colors: watchColors }),
      });
      ...
      if (result.status === "success") {
        ...
        setMessage(`❌ সমস্যা হয়েছে: ${result.message || "অজানা ত্রুটি"}`);
        return false;
      }
    } catch (error) {
      setMessage("❌ কানেকশনের সমস্যা হয়েছে।");
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const stockUpdated = await stockUpdate();
      ...
      setLoading(false);
      setResponse("Submission failed");
      console.error("Demand submission error:", err);
    }
  };
